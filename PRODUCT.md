...
- Prototype app: `C:\Users\user\Projects\wax1` — Svelte 5 + Vite, TailwindCSS + SCSS, SurrealDB backend, Discogs API integration (working). Reddit-scraping/Twilio/Resend alert pipeline exists in code but is not functional.
- DOGS parent brand: `C:\Users\user\Projects\wearedogs` — has logo, name, mission, and a general styling starter. Not yet reviewed in depth; treat as reference for the parent brand, not as Wax's identity.
