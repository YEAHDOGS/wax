 ...
 *
 * Live Resend/Twilio keys are NOT available yet (blocked on user), so
 ...
 *   the fully-wired adapters from `dispatch.js` (when user's keys are
 ...
 * `enabled: false` + `status: 'NOT WIRED'` until user supplies keys.
 */
...
function disabledReceipt(channel, needs, sentAt) {
  ...
  return {
    ...
    sentAt,
    error:
      `NOT WIRED — ${channel} sends are disabled until user supplies live keys ` +
 ...
 * dispatch through — real ones when user's keys are present, disabled
