 ...
 *   user-owned endpoint. Fully working.
 ...
 *   supplied ("needs API key from user"), and `send()` refuses to fake
 ...
 * delivery. Real provider wiring needs user's key AND a deliberate
...
export function createResendChannel({ apiKey, from } = {}) {
  if (!apiKey || typeof apiKey !== 'string') {
    throw new Error(
      'Resend email channel needs a live API key from user (RESEND_API_KEY) — ' +
...
export function createTwilioChannel({ accountSid, authToken, from } = {}) {
  if (!accountSid || !authToken) {
    throw new Error(
      'Twilio SMS channel needs live credentials from user ' +
