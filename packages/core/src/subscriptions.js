 ...
 * pending from user — the subscription is created anyway and a
 ...
 * keys still pending from user) or a throwing channel, the failure is
...
async function sendConfirmation({ channel, recipient, sub, confirmBaseUrl, now }) {
  ...
  if (!channel || typeof channel.send !== 'function') {
    return {
      ...
      channel: 'none',
      error:
        'provider keys pending — user has not supplied RESEND_API_KEY / ' +
