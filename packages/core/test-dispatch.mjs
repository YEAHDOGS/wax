...
test('createResendChannel: throws without a live key, names user as the source', () => {
  assert.throws(() => createResendChannel({}), /needs a live API key from user/);
  assert.throws(() => createResendChannel({ from: 'a@b.c' }), /needs a live API key from user/);
  assert.throws(() => createResendChannel({ apiKey: 're_x' }), /verified sender/);
});

test('createTwilioChannel: throws without live credentials, names user as the source', () => {
  assert.throws(() => createTwilioChannel({}), /needs live credentials from user/);
  assert.throws(() => createTwilioChannel({ accountSid: 'ACx' }), /needs live credentials from user/);
});
