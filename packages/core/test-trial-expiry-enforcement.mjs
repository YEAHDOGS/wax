...
test('effectivePlan: a trial past its expiry reads as free, even before the sweep', async () => {
  const past = new Date(now - 60_000).toISOString();
  assert.equal(effectivePlan(userRow('trial', past), { now }), 'free');
});

test('effectivePlan: a live trial keeps its plan, and the sweep agrees', async () => {
  const future = new Date(now + 60_000).toISOString();
  assert.equal(effectivePlan(userRow('trial', future), { now }), 'trial');
  const store = storeWithUser('trial', future);
  assert.equal(await expireTrials({ store }), 0, 'live trial is not swept');
  assert.equal(store.users.find((u) => u.id === 'usr_trial').plan, 'trial');
});

test('effectivePlan: expired trial matches exactly what the sweep will write', async () => {
  ...
  assert.equal(await expireTrials({ store }), 1, 'sweep downgrades the expired trial');
...
test('effectivePlan: series, free, unknown, and missing rows behave', async () => {
...
test('effectivePlan: a trial with no expiry date is not second-guessed', async () => {
  assert.equal(effectivePlan(userRow('trial', null), { now }), 'trial');
});

test('dispatch: an expired trial loses SMS — shouldDispatch refuses it like a free plan', async () => {
  const past = new Date(now - 60_000).toISOString();
...
test('dispatch: the same expired trial still gets email alerts', async () => {
  const past = new Date(now - 60_000).toISOString();
  const verdict = shouldDispatch({
    channel: 'email',
    plan: effectivePlan(userRow('trial', past), { now }),
...
test('watch gate: an expired-but-unswept trial hits the free watch limit', async () => {
  const past = new Date(now - 60_000).toISOString();
  const store = storeWithUser('trial', past);
  const token = store.createSession('usr_trial').token;
  for (let i = 0; i < FREE_WATCH_LIMIT; i++) {
    await addWatch(token, { artist_id: addArtist(store, i).id }, { store });
  }
  await assert.rejects(
    async () => await addWatch(token, { artist_id: addArtist(store, 99).id }, { store }),
    (err) => err.status === 402 && err.code === 'watch_limit',
    '4th watch on an expired trial is 402 watch_limit',
  );
});

test('source gate: an expired-but-unswept trial hits the free source limit', async () => {
  const past = new Date(now - 60_000).toISOString();
  const store = storeWithUser('trial', past);
  const token = store.createSession('usr_trial').token;
  ...
  await assert.rejects(
    async () => await addSource(token, { url: 'https://one-more-shop.example.com/' }, { store }),
    (err) => err.status === 402 && err.code === 'source_limit',
    'extra source on an expired trial is 402 source_limit',
  );
});

test('watch gate: a live trial is NOT capped', async () => {
