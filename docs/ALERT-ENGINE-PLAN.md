...
user's directive (2026-09-09): users add favorite artists AND favorite merch
...
Target cadence is every minute per user. Implementation is a scan queue
...
- Resend (email) + Twilio (SMS) — same providers the wax1 prototype aimed at.
  ...
  - **LIVE SENDS PENDING:** user still needs to supply `RESEND_API_KEY`
...
4. Artist matching + cross-source dedupe. — done (`scanner.js`, `rules.js`)
   ...
   - **BUILT (2026-09-09, jack/wax-collection-seed):** the collection
     ...
     Ships a synthetic `REDACTED-collection.json` fixture (8 entries,
     user-flavored — NOT real collection data, no token, no network).
   ...
   - **NEXT:** the actual Postgres `store.js` swap (live DB + `pg` driver)
     — needs user's call on hosting; everything above is written to make
   ...
   - **BUILT (2026-09-09, jack/wax-postgres-store):** that one file —
     ...
     already async). Remaining before production: user's `pg` install +
   ...
   - **NEXT:** the `pg` install (user's call — default-deny means the
...
5. Alert delivery (Resend, then Twilio). — done (adapters wired, key-gated; live keys pending from user)
6. UI: add artist, add site, scan status dashboard, alert history. — done (`dashboard.js`, `alert-history.js`)
   ...
   - **NEXT:** unsubscribe flow for email alerts (one-click List-Unsubscribe) — the missing piece
     before real sends, and it needs user's `RESEND_API_KEY` anyway.
